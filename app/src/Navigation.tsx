...
const Navigation = () => {
	...
	useEffect(() => {
		const cookie = Cookies.getCookie("id_token");
		console.log(cookie);
		setLoggedIn(cookie !== undefined ? true : false);
	}, []);
		...
		<Container>
			<div className="flex flex-col items-center justify-center gap-[40px] pt-[34px] w-full h-full bg-[rgba(0,123,236,0.1)]">
				...
				<div className="flex flex-col gap-[20px] max-w-[600px] w-full px-[10px]">
					...
					{loggedIn && <div className="text-center font-bold text-[18px]">You are logged in</div>}
